...
contract TestFactory is IFactory {
    address public owner;
    address public feePercentOwner;
    address public setStableOwner;
    address public feeTo;

    bytes32 public constant INIT_CODE_HASH = keccak256(abi.encodePacked(type(TestPair).creationCode));
     ...
     */
    modifier onlyOwner() {
        require(owner == msg.sender, "TSTFactory: caller is not the owner");
        _;
    }
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        require(tokenA != tokenB, 'TSTFactory: IDENTICAL_ADDRESSES');
        (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'TSTFactory: ZERO_ADDRESS');
        require(getPair[token0][token1] == address(0), 'TSTFactory: PAIR_EXISTS'); // single check is sufficient
        bytes memory bytecode = type(TestPair).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(token0, token1));
        assembly {
            pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        require(pair != address(0), "TSTFactory: FAILED");
        TestPair(pair).initialize(token0, token1);
    ...
    function setOwner(address _owner) external onlyOwner {
        require(_owner != address(0), "TSTFactory: zero address");
        emit OwnershipTransferred(owner, _owner);
        owner = _owner;
    }

    function setFeePercentOwner(address _feePercentOwner) external onlyOwner {
        require(_feePercentOwner != address(0), "TSTFactory: zero address");
    ...
    function setSetStableOwner(address _setStableOwner) external {
        require(msg.sender == setStableOwner, "TSTFactory: not setStableOwner");
        require(_setStableOwner != address(0), "TSTFactory: zero address");
    ...
    function setOwnerFeeShare(uint newOwnerFeeShare) external onlyOwner {
        require(newOwnerFeeShare > 0, "TSTFactory: ownerFeeShare mustn't exceed minimum");
        require(newOwnerFeeShare <= OWNER_FEE_SHARE_MAX, "TSTFactory: ownerFeeShare mustn't exceed maximum");
    ...
    function setReferrerFeeShare(address referrer, uint referrerFeeShare) external onlyOwner {
        require(referrer != address(0), "TSTFactory: zero address");
        require(referrerFeeShare <= REFERER_FEE_SHARE_MAX, "TSTFactory: referrerFeeShare mustn't exceed maximum");
