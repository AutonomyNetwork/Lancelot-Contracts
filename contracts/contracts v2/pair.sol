...
contract TestPair is IPair, UniswapV2ERC20 {
  ...
  function initialize(address _token0, address _token1) external {
    require(msg.sender == factory && !initialized, 'TestPair: FORBIDDEN');
    // sufficient check
    token0 = _token0;
    token1 = _token1;
  ...
  */
  function setFeePercent(uint16 newToken0FeePercent, uint16 newToken1FeePercent) external lock {
    require(msg.sender == IFactory(factory).feePercentOwner(), "TestPair: only factory's feeAmountOwner");
    require(newToken0FeePercent <= MAX_FEE_PERCENT && newToken1FeePercent <= MAX_FEE_PERCENT, "TestPair: feePercent mustn't exceed the maximum");
    require(newToken0FeePercent > 0 && newToken1FeePercent > 0, "TestPair: feePercent mustn't exceed the minimum");
  ...
  function setStableSwap(bool stable, uint112 expectedReserve0, uint112 expectedReserve1) external lock {
    require(msg.sender == IFactory(factory).setStableOwner(), "TestPair: only factory's setStableOwner");
    require(!pairTypeImmutable, "TestPair: immutable");

    require(stable != stableSwap, "TestPair: no update");
    require(expectedReserve0 == reserve0 && expectedReserve1 == reserve1, "TestPair: failed");
  ...
  function setPairTypeImmutable() external lock {
    require(msg.sender == IFactory(factory).owner(), "TestPair: only factory's owner");
    require(!pairTypeImmutable, "TestPair: already immutable");
  ...
  function _swap(TokensData memory tokensData, address to, bytes memory data, address referrer) internal lock {
    require(tokensData.amount0Out > 0 || tokensData.amount1Out > 0, 'TestPair: INSUFFICIENT_OUTPUT_AMOUNT');

    (uint112 _reserve0, uint112 _reserve1, uint16 _token0FeePercent, uint16 _token1FeePercent) = getReserves();
    require(tokensData.amount0Out < _reserve0 && tokensData.amount1Out < _reserve1, 'TestPair: INSUFFICIENT_LIQUIDITY');


    {
      require(to != tokensData.token0 && to != tokensData.token1, 'TestPair: INVALID_TO');
    ...
    {// scope for reserve{0,1}Adjusted, avoids stack too deep errors
      ...
      require(_k(balance0Adjusted, balance1Adjusted) >= _k(uint(_reserve0), uint(_reserve1)), 'TestPair: K');
    }
  ...
  function sync() external lock {
    ...
    require(token0Balance != 0 && token1Balance != 0, "TestPair: liquidity ratio not initialized");
  ...
  */
  function drainWrongToken(address token, address to) external lock {
    require(msg.sender == IFactory(factory).owner(), "TestPair: only factory's owner");
    require(token != token0 && token != token1, "TestPair: invalid token");
